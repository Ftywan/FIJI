...
public class BlockNestedJoinOriginal extends Join {
	...
	boolean eobj; // end of the// nested join
	
	int outerTupleTracker; //track the status of outer relation
	public BlockNestedJoinOriginal(Join jn) {
			...
			if (eosr == true && eosl == false) {
				...
				for (int i = 0; i < (numBuff - 2); i++) {
					...
					for (int j = 0; j < outerBatch.size(); j++) {
						/** 
						 * store the tuple in the hashmap using the searchKey as a key value
						 * **/
						System.out.println("Loading tuples no : " + ++outerTupleTracker + " into memory");
						Tuple outerTuple = outerBatch.get(j);
